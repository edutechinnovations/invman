                    ...
                    <div class="in-bold">
                        @if (count($paidToDate))
                            @foreach ($paidToDate as $item)
                                {{ App\Libraries\Utils::formatMoney($item->value, $item->currency_id) }}<br/>
                            @endforeach
                        @else
                            {{ App\Libraries\Utils::formatMoney(0) }}
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div class="in-bold">
                        @if (count($averageInvoice))
                            @foreach ($averageInvoice as $item)
                                {{ App\Libraries\Utils::formatMoney($item->invoice_avg, $item->currency_id) }}<br/>
                            @endforeach
                        @else
                            {{ App\Libraries\Utils::formatMoney(0) }}
                        @endif
        ...
        <div class="panel panel-default dashboard" style="height:320px">
            ...
            <ul class="panel-body list-group" style="height:276px;overflow-y:auto;">
                ...
                <li class="list-group-item">
                    <span style="color:#888;font-style:italic">{{ App\Libraries\Utils::timestampToDateString(strtotime($activity->created_at)) }}:</span>
                    {!! $activity->getMessage() !!}
                </li>
                @endforeach
            </ul>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @endif
                            <td>{{ App\Libraries\Utils::fromSqlDate($payment->payment_date) }}</td>
                            <td>{{ App\Libraries\Utils::formatMoney($payment->amount, $payment->currency_id ?: ($account->currency_id ?: DEFAULT_CURRENCY)) }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
                        ...
                        @foreach ($upcoming as $invoice)
                            @if (!$invoice->is_quote)
                                <tr>
                                    ...
                                    <td>{{ App\Libraries\Utils::formatMoney($invoice->balance, $invoice->currency_id ?: ($account->currency_id ?: DEFAULT_CURRENCY)) }}</td>
                                </tr>
                            @endif
                        @endforeach
                    </tbody>
                            ...
                            @if (!$invoice->is_quote)
                                <tr>
                                    ...
                                    @else
                                        <td>{{ trim($invoice->client_name) ?: (trim($invoice->first_name . ' ' . $invoice->last_name) ?: $invoice->email) }}</td>
                                    @endif
                                    <td>{{ App\Libraries\Utils::fromSqlDate($invoice->due_date) }}</td>
                                    <td>{{ App\Libraries\Utils::formatMoney($invoice->balance, $invoice->currency_id ?: ($account->currency_id ?: DEFAULT_CURRENCY)) }}</td>
                                </tr>
                            @endif
                        @endforeach
